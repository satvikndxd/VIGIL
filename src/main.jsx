...
function Dashboard() {
  ...
  const classCounts = Object.keys(labels).map(k => ({ key: k, value: records.filter(r => r.true_class === k).length }));
  const matrix = Object.keys(labels).map(a => Object.keys(labels).map(b => records.filter(r => r.true_class === a && r.pred_class === b).length));
