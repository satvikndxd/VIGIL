...
phase3=json.loads((OUT/'phase3_metrics.json').read_text()); cfg=json.loads((OUT/'phase3_best_config.json').read_text()); rep=pd.read_csv(OUT/'phase3_representation_ablation.csv'); sym=pd.read_csv(OUT/'phase3_symbol_balance.csv'); mt=pd.read_csv(OUT/'phase3_multitask.csv'); dom=pd.read_csv(OUT/'phase3_domain_adversarial.csv'); seeds=pd.read_csv(OUT/'phase3_seed_results.csv'); boot=pd.read_csv(OUT/'phase3_bootstrap_results.csv'); cal=pd.read_csv(OUT/'phase3_calibration.csv'); rob=pd.read_csv(OUT/'phase3_robustness.csv'); rec=pd.read_csv(OUT/'phase3_record_generalization.csv')

def table(df,cols=None):
    return (df if cols is None else df[cols]).round(4).to_markdown(index=False)
seed_base=seeds[seeds.model=='frozen_baseline'];seed_p3=seeds[seeds.model=='phase3_symbol_loss']
summary=seed_base.agg({'MacroF1':['mean','std'],'MacroAUPRC':['mean','std'],'MacroAUROC':['mean','std'],'N_recall':['mean','std'],'F_recall':['mean','std']})
report=f'''# VIGIL Phase 3 — Domain-Generalized ECG Representation Learning
...
|---|---:|---:|---:|---:|---:|
{table(sym[['experiment','MacroF1','MacroAUPRC','MacroAUROC','N_recall','F_recall']])}
...
{table(mt[['lambda','MacroF1','MacroAUPRC','MacroAUROC','N_recall','F_recall','validation_MacroF1']])}
...
{table(rep[['representation','MacroF1','MacroAUPRC','MacroAUROC','N_recall','F_recall']])}
...
|---|---:|---:|---:|---:|---:|---:|
{table(cal[['model','temperature','ECE','Brier','MacroF1','MacroAUPRC','MacroAUROC']])}
...
The Phase 3 symbol-loss candidate was substantially less stable and lower-performing across seeds. Its mean Macro-F1 was {seed_p3.MacroF1.mean():.4f} ± {seed_p3.MacroF1.std():.4f}, with mean N recall {seed_p3.N_recall.mean():.4f} ± {seed_p3.N_recall.std():.4f} and F recall {seed_p3.F_recall.mean():.4f} ± {seed_p3.F_recall.std():.4f}.

## Bootstrap and robustness

The final retained model bootstrap intervals use 500 resamples:

{table(boot[['metric','estimate','ci_lower_95','ci_upper_95','bootstrap_samples']])}
...
'''
(OUT/'phase3_report.md').write_text(report)
print(OUT/'phase3_report.md')
